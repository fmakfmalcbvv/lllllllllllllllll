    ...
    treeListener(treeClass, function(tree)
        ...
        spawn(function()
            for i = 1, 60 do
                ...
                tree:PivotTo(daoting.treecutset)
                game["Run Service"].Heartbeat:wait()
            end
        end)
...
local function autofarm(treeClass)
    ...
    until treeCut
    if daoting.autofarm1 == false then
        notify("道庭", "完成", 3)
    end
    tp(oldPosition)
end
...
local function donate(plr, amount)
    ...
    if daoting.donationRecipient == nil or not game:GetService('Players'):FindFirstChild(plr) then
        notify('错误', '没有找到玩家', 4);
        return;
    end
    if tonumber(daoting.moneyaoumt) >= 60000000 then
        notify('错误', '数字太高', 4);
        return;
    end
    if tonumber(daoting.moneyaoumt) <= 0 then
        notify('错误', '数字太高', 4);
        return;
    end
    ...
    if tonumber(daoting.moneyaoumt) > lp.leaderstats.Money.Value then
        notify('错误', '你没有足够的钱', 4);
        return;
    end
    ...
    local oldAmount = daoting.Players:FindFirstChild(plr).leaderstats.Money.Value;
    ...
    notify('道庭', '转钱' .. tostring(amount) .. ' 给 ' .. tostring(plr), 4);
    scr2.exitNotice();
end
...
local function farAxeEquip()
    ...
    local done = false;
    if daoting.farAxeEquip == nil then
        notify('道庭', '选择一把斧头', 4);
        daoting.farAxeEquip = Mouse.Button1Down:connect(function()
        ...
        until done;
        notify('道庭', '已装备', 4);
        if daoting.farAxeEquip then
            daoting.farAxeEquip:Disconnect();
            daoting.farAxeEquip = nil;
        end
    else
        notify('错误', '已经激活', 4);
    end
...
local function applyLight(value)
    if value then
        ...
        pcall(function()
            lp.Character.Head.daoting:remove();
        end);
    end
end
...
local function CheckSlotNumber() -- Checks if the slot number is right
    if daoting.soltnumber == "1" or daoting.soltnumber == "2" or daoting.soltnumber == "3" or daoting.soltnumber == "4" or
        daoting.soltnumber == "5" or daoting.soltnumber == "6" then
        local SlotNumber = tonumber(daoting.soltnumber)
        return SlotNumber
    else
        return false
    end
...
local function PlankToBlueprint()
    ...
    local target;
    notify('道庭', '选择一个木头和蓝图', 2);
    daoting.PlankToBlueprint = game:GetService('Players').LocalPlayer:GetMouse().Button1Down:Connect(function()
        ...
        if target.Parent:FindFirstChild('Type') and target.Parent.Type.Value == 'Blueprint' then
            daoting.blueprintModel = game:GetService('Players').LocalPlayer:GetMouse().Parent;
            notify('道庭', '蓝图已选择', 2);
        end
        if tostring(target.Parent) == 'Plank' and target.Parent:FindFirstChild('Owner') and
            ...
            daoting.plankModel = target.Parent;
            notify('道庭', '木头已选择', 2);
        end
    end);
    repeat
        wait()
    until daoting.plankModel and daoting.blueprintModel;
    daoting.PlankToBlueprint:Disconnect();
    daoting.PlankToBlueprint = nil;
    tp(CFrame.new(daoting.plankModel:FindFirstChildOfClass 'Part'.CFrame.p + Vector3.new(0, 3, 4)))
    wait(.2)
    for i = 1, 30 do
        pcall(function()
            game:GetService('ReplicatedStorage').Interaction.ClientIsDragging:FireServer(daoting.plankModel)
            daoting.plankModel.WoodSection.CFrame = CFrame.new(daoting.blueprintModel.Main.CFrame.p + Vector3.new(0, 1.5, 0));
            game:GetService 'RunService'.Stepped:wait();
        end);
    end

    notify('道庭', '完成', 2);
    daoting.blueprintModel = nil;
    daoting.plankModel = nil;
end
...
function axefily()
    ...
    daoting.axeFling = mouse.Button1Down:Connect(function()
        local axe = nil
        ...
        spawn(function()
            while daoting.whthmose == true do
                task.wait(0.1)
                fling.Position = lp:GetMouse().Hit.p;
            end
        end)
...
function autobuy(o, r)
    daoting.autocsdx = game.Workspace.PlayerModels.ChildAdded:connect(function(v)
        v:WaitForChild('Owner', 60)
        if v.Owner.Value == lp then
            for i = 1, 20 do
                game:GetService("ReplicatedStorage").Interaction.ClientIsDragging:FireServer(v)
                v:PivotTo(daoting.autobuyset)
                game:GetService("ReplicatedStorage").Interaction.ClientIsDragging:FireServer(v)
                game:GetService('RunService').Stepped:wait();
            end
        end
    ...
    for e = 1, r do
        if daoting.autobuystop == false then
            autobuyv2(o)
            task.wait()
        end
    end
    spawn(function()
        wait(1)
        daoting.autocsdx:Disconnect();
        daoting.autocsdx = nil;
    end)
    return o, r
end
...
function shuaxinlb(zji)
    ...
    if zji == true then
        for p, I in next, game.Players:GetChildren() do
            table.insert(daoting.dropdown, I.Name)
        end
    else
        for p, I in next, game.Players:GetChildren() do
            if I ~= lp then
...
local win = library:new("道庭--伐木大亨2", '')
...
local Tab = win:Tab("斧头", "14116409053")
local Section = Tab:section("斧头", true)
local Tab6 = win:Tab("基地", "14116406854")
local Section1 = Tab6:section("基地", false)
local Tab7 = win:Tab("木头", "14116411212")
local Section5 = Tab7:section("木头", true)
local Sectionbringtree = Tab7:section("带来树", true)
local Sectionjinbi = Tab1:section("快速转钱", false)
local Sectionqiche = Tab1:section("汽车", false)
local Tab8 = win:Tab("传送物品", "14116575656")
local Sectionchuanson = Tab8:section("传送物品", true)
local Sectionfengche = Tab1:section("刷粉车", false)
local Sectionshuoh = Tab1:section("语言", false)
local Sectionyule = Tab1:section("娱乐区域", false)
local Tab10 = win:Tab("整理", "14116575656")
local Sectionzhenglimt = Tab10:section("整理木板", false)
local Sectionzhengli = Tab10:section("整理物品", true)
local Sectiontchon = Tab7:section("填充蓝图（用木头）", false)
...
local Tab9 = win:Tab("其他功能", "14116416084")
local Sectionqita = Tab9:section("主要功能", false)
local Sectionqita1 = Tab9:section("捷径", true)
local Sectionqita2 = Tab9:section("其他", false)

Section:Toggle("自动扔斧头", 'Toggleflag', false, function(state)
    daoting.autodropae = true
    if state then

        while wait() do

            if daoting.autodropae == true then
                local oldpos = lp.Character.HumanoidRootPart.CFrame
                droptool(oldpos)
            end
        end
    else
        daoting.autodropae = false
    end
end)
Section:Toggle("自动捡斧头", 'Toggleflag', false, function(state)
    if state then
        daoting.autopick = true
        while daoting.autopick == true do
        ...
        daoting.autopick = false
    end
end)

Section:Button("加载复制斧头(加载冷却过后用,否则有bug)", function()
    CanClientLoad()
    wait(1)
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(273, -350, -98)
    wait(2)
...
Section:Toggle("斧头跟随鼠标", 'Toggleflag', false, function(state)
    if state then
        daoting.whthmose = true
    else
        daoting.whthmose = false
    end
end)
Section:Toggle("斧头炸家", 'Toggleflag', false, function(state)
    if state then
        ...
        if daoting.axeFling then
            daoting.axeFling:Disconnect(0.1);
            daoting.axeFling = nil;
        end
    end
end)
Section1:Button("点击土地免费获得", function()
    freeland=nil
    notify("道庭","请你点击一个空的土地",4)
...
DupeSlot=tonumber(daoting.soltnumber)
...
Sectionbringtree:Dropdown("选择树", 'Dropdown',
    ...
    function(b)
        if b == '普通树' then
            daoting.cuttreeselect = "Generic"
        elseif b == '沼泽黄金' then
            daoting.cuttreeselect = "GoldSwampy"
        elseif b == '樱花' then
            daoting.cuttreeselect = "Cherry"
        elseif b == '蓝木' then
            daoting.cuttreeselect = "CaveCrawler"
        elseif b == '冰木' then
            daoting.cuttreeselect = "Frost"
        elseif b == '火山木' then
            daoting.cuttreeselect = "Volcano"
        elseif b == '橡木' then
            daoting.cuttreeselect = "Oak"
        elseif b == '巧克力木' then
            daoting.cuttreeselect = "Walnut"
        elseif b == '白桦木' then
            daoting.cuttreeselect = "Birch"
        elseif b == '黄金木' then
            daoting.cuttreeselect = "SnowGlow"
        elseif b == '雪地松' then
            daoting.cuttreeselect = "Pine"
        elseif b == '僵尸木' then
            daoting.cuttreeselect = "GreenSwampy"
        elseif b == '大巧克力树' then
            daoting.cuttreeselect = "Koa"
        elseif b == '椰子树' then
            daoting.cuttreeselect = "Palm"
        elseif b == '幽灵木' then
            daoting.cuttreeselect = "Spooky"
        elseif b == '南瓜木' then
            daoting.cuttreeselect = "SpookyNeon"
        elseif b == '幻影木' then
            daoting.cuttreeselect = "LoneCave"
        end
    end)

Sectionbringtree:Textbox("带来树的数量", 'TextBoxfalg', "1", function(txt)
    daoting.bringamount = txt
end)
Sectionbringtree:Button("带来树", function()
    daoting.bringtree = true
    daoting.treecutset = lp.Character.HumanoidRootPart.CFrame
    task.wait(0.2)
    for i = 1, daoting.bringamount do
        if daoting.bringtree == true then
            task.wait()
            bringTree(daoting.cuttreeselect)
        end
...
Sectionbringtree:Button("停止", function()
    daoting.bringtree = false
end)
Sectionbringtree:Toggle("自动砍树", 'Toggleflag', false, function(state)

    if state then
        daoting.autofarm = true
        task.spawn(function()
            while task.wait(0.3) do

                if daoting.autofarm == true then
...
Sectionshuoh:Textbox("说话次数", 'TextBoxfalg', "输入数字", function(txt)
    daoting.saymount = txt
end)
Sectionshuoh:Button("说话", function()
    daoting.sayfast = true
    for i = 1, daoting.saymount do
        if daoting.sayfast == true then
            game:GetService('ReplicatedStorage').DefaultChatSystemChatEvents.SayMessageRequest:FireServer(daoting.saymege,
                'All')
        end
    end
end)
Sectionshuoh:Button("停止说话", function()
    daoting.sayfast = false
end)
Sectionshuoh:Toggle("全自动说话", 'Toggleflag', false, function(state)
    if state then
        daoting.autosay = true
        while task.wait() do
            if daoting.autosay == true then
        ...
        daoting.autosay = false
    end
end)
local dropdown = Sectionjinbi:Dropdown("选择玩家名字", 'Dropdown', daoting.dropdown, function(v)
    daoting.moneytoplayername = v
end)

Sectionjinbi:Button("刷新列表", function()
    shuaxinlb(true)
    dropdown:SetOptions(daoting.dropdown)
end)
Sectionjinbi:Textbox("给玩家转钱数量", 'TextBoxfalg', "输入数量", function(txt)
    daoting.moneyaoumt = txt
end)

Sectionjinbi:Button("开始转钱", function()
    donate(daoting.moneytoplayername, daoting.moneyaoumt)
end)
...
Sectionqita:Toggle("远程打开东西", 'Toggleflag', false, function(state)
    if state then
        notify('道庭', '选择一个东西去打开', 4)
        daoting.openItem = mouse.Button1Down:Connect(function()
            if mouse.Target then
                daoting.itemtoopen = mouse.Target;
            end
            OpenSelectedItem(daoting.itemtoopen.Parent);
        end)
    else
        if daoting.openItem then
            daoting.openItem:Disconnect();
            daoting.openItem = nil;
        end
        notify('道庭', '打开东西已关闭', 4)
        daoting.itemToOpen = nil;
    end
end)
...
Sectionqita1:Button("生成上火山捷径", function()
...
Sectionqita1:Button("生成去椰子岛的捷径", function()
...
Sectionqita1:Button("沼泽捷径", function()
...
Sectionqita1:Button("黄金木捷径", function()
...
Sectionqita1:Button("冰木捷径", function()
    local f0 = Instance.new("Folder", workspace)
    f0.Name = "FrostPath"
    local part = Instance.new("Part", f0)
...
Sectionqita2:Button("更简单的砍幻影", function()
...
Sectionqita2:Button("点击传送", function()
...
Sectionqita2:Button("重进服务器", function()
...
Sectionyule:Button("安全自杀--旧(会掉斧头)", function()
    lp.Character.Head:Destroy()
end)

Sectionyule:Button("安全自杀--新(不会掉斧头)", function()
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(273, -350, -98)
end)
local Players = Sectionzhenglimt:Dropdown("选择玩家", 'Dropdown', daoting.dropdown, function(v)
    daoting.mtwjia = v
end)

Sectionzhenglimt:Button("重置玩家名称", function()
    shuaxinlb(true)
    Players:SetOptions(daoting.dropdown)
end)
local drop = Sectionzhenglimt:Dropdown("选择木头类型", 'Dropdown',
    ...
    function(b)
        if b == '普通树' then
            daoting.zlmt = "Generic"
        elseif b == '沼泽黄金' then
            daoting.zlmt = "GoldSwampy"
        elseif b == '樱花' then
            daoting.zlmt = "Cherry"
        elseif b == '蓝木' then
            daoting.zlmt = "CaveCrawler"
        elseif b == '冰木' then
            daoting.zlmt = "Frost"
        elseif b == '火山木' then
            daoting.zlmt = "Volcano"
        elseif b == '橡木' then
            daoting.zlmt = "Oak"
        elseif b == '巧克力木' then
            daoting.zlmt = "Walnut"
        elseif b == '白桦木' then
            daoting.zlmt = "Birch"
        elseif b == '黄金木' then
            daoting.zlmt = "SnowGlow"
        elseif b == '雪地松' then
            daoting.zlmt = "Pine"
        elseif b == '僵尸木' then
            daoting.zlmt = "GreenSwampy"
        elseif b == '大巧克力树' then
            daoting.zlmt = "Koa"
        elseif b == '椰子树' then
            daoting.zlmt = "Palm"
        elseif b == '幻影' then
            daoting.zlmt = "LoneCave"
        elseif b == '幽灵木' then
            daoting.zlmt = "Spooky"
        elseif b == '南瓜木' then
            daoting.zlmt = "SpookyNeon"
        end
...
Sectionzhenglimt:Toggle("改为竖着整理", 'Toggleflag', false, function(state)
    if state then
        ...
        daoting.shuzhe = false

    end
end)
Sectionzhenglimt:Button("开始整理", function()
    if daoting.zlmt == nil then
        return notify("道庭", "你没有选择木头", 4)
    end
    if daoting.shuzhe == false then
        local oldpos = lp.Character.HumanoidRootPart.Position

        for _, Plank in pairs(game.Workspace.PlayerModels:GetChildren()) do
            if Plank.Name == "Plank" and Plank:findFirstChild("Owner") then
                if Plank:FindFirstChild("Owner") and tostring(Plank.Owner.Value) == daoting.mtwjia then
                    if Plank.TreeClass.Value == daoting.zlmt then
...
local drop = Sectiontchon:Dropdown("选择木头类型", 'Dropdown',
    {"普通树", "沼泽黄金", "樱花", "蓝木", "冰木", "火山木", "橡木", "巧克力木", "白桦木",
     "黄金木", "雪地松", "僵尸木", "大巧克力树", "椰子树", '幻影'}, function(b)
        if b == '普通树' then
            daoting.tchonmt = "Generic"
        elseif b == '沼泽黄金' then
            daoting.tchonmt = "GoldSwampy"
        elseif b == '樱花' then
            daoting.tchonmt = "Cherry"
        elseif b == '蓝木' then
            daoting.tchonmt = "CaveCrawler"
        elseif b == '冰木' then
            daoting.tchonmt = "Frost"
        elseif b == '火山木' then
            daoting.tchonmt = "Volcano"
        elseif b == '橡木' then
            daoting.tchonmt = "Oak"
        elseif b == '巧克力木' then
            daoting.tchonmt = "Walnut"
        elseif b == '白桦木' then
            daoting.tchonmt = "Birch"
        elseif b == '黄金木' then
            daoting.tchonmt = "SnowGlow"
        elseif b == '雪地松' then
            daoting.tchonmt = "Pine"
        elseif b == '僵尸木' then
            daoting.tchonmt = "GreenSwampy"
        elseif b == '大巧克力树' then
            daoting.tchonmt = "Koa"
        elseif b == '椰子树' then
            daoting.tchonmt = "Palm"
        elseif b == '幻影' then
            daoting.tchonmt = "LoneCave"
        end
    end)
...
Sectiontchon:Button("填充蓝图（木头）", function()
    ...
    tool.Activated:Connect(function()
        ...
        if str:FindFirstChild("Type") and str.Type.Value == "Blueprint" and str:FindFirstChild("Owner") then
            lumbsmasher_legitpaint(daoting.tchonmt, str, true)
        end
    end)
end)
    ...
    for i, v in pairs(game.Workspace.PlayerModels:GetChildren()) do
        ...
        if v:FindFirstChild("Type") and v.Type.Value == "Blueprint" and v:FindFirstChild("Owner") then
            if v.Owner.Value == lp then
                ...
                lumbsmasher_legitpaint(daoting.tchonmt, v, true)
...
Section3:Slider('设置速度', 'Sliderflag', 16, 16, 600, false, function(s)
    ...
    spawn(function()
        while task.wait() do
            game:GetService("Players").LocalPlayer.Character.Humanoid.WalkSpeed = daoting.walkspeed
        end
    end)
end)
Section3:Slider('设置跳跃', 'Sliderflag', 50, 50, 600, false, function(s)
    ...
    spawn(function()
        while task.wait() do
            game:GetService("Players").LocalPlayer.Character.Humanoid.JumpPower = daoting.JumpPower
        end
    end)
end)
...
Section3:Button("安全自杀--旧(会掉斧头)", function()
    lp.Character.Head:Destroy()
end)
Section3:Button("安全自杀--新(不会掉斧头)", function()
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(273, -350, -98)
end)
...
Sectiontptree:Dropdown("传送到树", 'Dropdown',
    {"普通树", "沼泽黄金", "樱花", "蓝木", "冰木", "火山木", "橡木", "巧克力木", "白桦木",
     "黄金木", "雪地松", "僵尸木", "大巧克力树", "椰子树", "幻影木"}, function(b)
        if b == '普通树' then
            daoting.tptree = "Generic"
        elseif b == '沼泽黄金' then
            daoting.tptree = "GoldSwampy"
        elseif b == '樱花' then
            daoting.tptree = "Cherry"
        elseif b == '蓝木' then
            daoting.tptree = "CaveCrawler"
        elseif b == '冰木' then
            daoting.tptree = "Frost"
        elseif b == '火山木' then
            daoting.tptree = "Volcano"
        elseif b == '橡木' then
            daoting.tptree = "Oak"
        elseif b == '巧克力木' then
            daoting.tptree = "Walnut"
        elseif b == '白桦木' then
            daoting.tptree = "Birch"
        elseif b == '黄金木' then
            daoting.tptree = "SnowGlow"
        elseif b == '雪地松' then
            daoting.tptree = "Pine"
        elseif b == '僵尸木' then
            daoting.tptree = "GreenSwampy"
        elseif b == '大巧克力树' then
            daoting.tptree = "Koa"
        elseif b == '椰子树' then
            daoting.tptree = "Palm"
        elseif b == '幻影木' then
            daoting.tptree = "LoneCave"
        end
        for i, v in pairs(game.Workspace:GetChildren()) do
            if v.Name == "TreeRegion" then
                for j, k in ipairs(v:GetChildren()) do
                    if k:FindFirstChild("TreeClass") and k.TreeClass.Value == daoting.tptree then
...
Sectionhuanjin:Toggle("终日白天", 'Toggleflag', false, function(state)
    if state then
        daoting.awaysday = true
        while task.wait() do
            if daoting.awaysday == true then
                game:GetService('Lighting').TimeOfDay = ('12:00:00');
            end
        end
    else
        daoting.awaysday = false
    end

end)
Sectionhuanjin:Toggle("终日黑天", 'Toggleflag', false, function(state)
    if state then
        daoting.awaysdnight = true
        while task.wait() do
            if daoting.awaysdnight == true then
                game:GetService('Lighting').TimeOfDay = ('2:00:00');
            end
        end
    else
        daoting.awaysdnight = false
    end
end)
...
Sectionhuanjin:Toggle("水上行走", 'Toggleflag', false, function(state)
    for i, v in next, game.workspace.Water:children() do
        if v.ClassName == 'Part' then
            daoting.waterwalk, v.CanCollide = state, state;
        end
    end
...
local Players = Sectionzhengli:Dropdown("选择玩家", 'Dropdown', daoting.dropdown, function(v)
    daoting.zlwjia = v
end)
...
Sectionzhengli:Button("获取整理工具", function()
    ...
    Identify.Activated:connect(function()
        ...
        if mouse.Target.Parent:FindFirstChild("PurchasedBoxItemName") then
            daoting.dxmz = (mouse.Target.Parent.PurchasedBoxItemName.Value)

            function ItemStacker(ItemType, XAxis, ZAxis)
                for i, v in pairs(game:GetService("Workspace").PlayerModels:GetChildren()) do
                    if v:FindFirstChild("Owner") and tostring(v.Owner.Value) == daoting.zlwjia then
        ...
        elseif mouse.Target.Parent:FindFirstChild("ItemName") then
            ...
            ItemStackerft(daoting.dxmz, daoting.zlz, daoting.zix)
            notify('', '' .. mouse.Target.Parent.ItemName.Value, 5)
        end
    end)
    Identify.Parent = game.Players.LocalPlayer.Backpack
end)
local Players = Sectionchuanson:Dropdown("选择玩家", 'Dropdown', daoting.dropdown, function(v)
    daoting.cswjia = v
end)
...
Sectionchuanson:Button("设置传送点", function()
    pcall(function()
        game.Workspace.daotingBasedropCord:Destroy();
        daoting.itemset = nil
    end)
    local daotingBasedropCord = Instance.new("Part", game.Workspace)
    daotingBasedropCord.CanCollide = false
    daotingBasedropCord.Anchored = true
    daotingBasedropCord.Shape = Enum.PartType.Ball
    daotingBasedropCord.Color = Color3.fromRGB(0, 217, 255);
    daotingBasedropCord.Transparency = 0
    daotingBasedropCord.Size = Vector3.new(2, 2, 2)
    daotingBasedropCord.CFrame = lp.Character.HumanoidRootPart.CFrame
    daotingBasedropCord.Material = Enum.Material.Marble
    daotingBasedropCord.Name = "daotingBasedropCord"

    daoting.itemset = lp.Character.HumanoidRootPart.CFrame
end)
...
Sectionchuanson:Button("获得工具", function()
    if daoting.itemset == nil then
        return notify("道庭", "请你放传送点", 4)
    end
    ...
    Tool.Activated:connect(function()
        ...
        for _, v in next, workspace.PlayerModels:children() do
            ...
            local main
            if daoting.cskais == true then

                if check and check.Value == daoting.dxmz and v:FindFirstChild('Owner') and tostring(v.Owner.Value) ==
                    daoting.cswjia or check2 and check2.Value == daoting.dxmz and v:FindFirstChild('Owner') and
                    tostring(v.Owner.Value) == daoting.cswjia then
...
Sectionchuanson:Toggle("点击选择传送物品", 'Toggleflag', false, function(state)
    if state then
        ClickToSelectClick = Mouse.Button1Up:Connect(function()
            Clicked = Mouse.Target
            if Clicked.Parent:FindFirstChild("Owner") and tostring(Clicked.Parent.Owner.Value) == daoting.cswjia then
...
Sectionchuanson:Button("取消选择", function()
    for i, v in next, game:GetService("Workspace").PlayerModels:GetChildren() do
        if v:FindFirstChild("Owner") and tostring(v.Owner.Value) == daoting.cswjia then
...
Sectionchuanson:Toggle("框选物品", 'Toggleflag', false, function(state)
    if state then
        daoting.kuangxiu = game:GetService("UserInputService").InputBegan:Connect(function(cilk)
            ...
            if cilk.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                while game:GetService("UserInputService"):IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    ...
                    for i, v in pairs(workspace.PlayerModels:GetChildren()) do
                        if daoting.xzemuban == true and v:FindFirstChild("Owner") and tostring(v.Owner.Value) == daoting.cswjia and
                        ...
                        if v:FindFirstChild("Owner") and tostring(v.Owner.Value) == daoting.cswjia and
        ...
        Frame.Visible = false
        daoting.kuangxiu:Disconnect()
        daoting.kuangxiu = nil
    end

end)
Sectionchuanson:Toggle("带木板", 'Toggleflag', false, function(state)

    daoting.xzemuban = state
            ...
            if v:FindFirstChild("SelectionBox") then
                ...
                elseif v:FindFirstChild("WoodSection") then
                    ...
                    for e = 1, 70 do
                        ...
                        game:GetService("ReplicatedStorage").Interaction.ClientIsDragging:FireServer(v)
                        v.WoodSection.CFrame = daoting.itemset * CFrame.Angles(math.rad(90), 0, 90)
                        game:GetService("ReplicatedStorage").Interaction.ClientIsDragging:FireServer(v)
                        game:GetService('RunService').Stepped:wait();
                    end
                    v.SelectionBox:Destroy()
...
Sectionhuanjin:Button("船离开时间", function()
    ...
    if game.Workspace.Ferry.TimeToDeparture.Value == 0 then
        ...
        notify("道庭", "距离船离开时间有" .. game.Workspace.Ferry.TimeToDeparture.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("桥上升时间", function()
    ...
    if game.Workspace.Bridge.DownTime.Value < 0 then
        ...
        notify("道庭", "距离桥上升的时间有" .. game.Workspace.Bridge.DownTime.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("查看游戏时间", function()
    notify("道庭", "游戏时间为" .. tostring(game.Lighting.TimeOfDay):sub(1, -2):sub(1, -2):sub(1, -2), 4)
end)
    ...
    Identify.Activated:connect(function()
        if Mouse.Target and Mouse.Target.Parent.Type and Mouse.Target.Parent.Type.Value == "Vehicle Spot" then
            if not Mouse.Target.Parent:FindFirstChild("SelectionBox") then
                daoting.car = Mouse.Target.Parent:FindFirstChild("ButtonRemote_SpawnButton", true)
...
Sectionfengche:Button("开始", function()
    ...
    if daoting.car ~= nil then
        repeat
            ...
            until FP ~= C
            C = FP
        until FP.BrickColor.Name == "Hot pink" or daoting.stopcar == true
        a:Disconnect();
        a = nil
    else
        notify("道庭", "你暂时没有选择汽车", 4)
    end
end)
Sectionfengche:Button("停止", function()
    daoting.stopcar = true
    daoting.car = nil
    for i, v in next, game:GetService("Workspace").PlayerModels:GetChildren() do
...
local dropdown = Sectionmogui:Dropdown("选择玩家名称", 'Dropdown', daoting.dropdown, function(v)
    daoting.playernamedied = v
end)
...
Sectionmogui:Button("传送到玩家旁边", function()
    ...
    local tp_player = game:GetService("Players")[daoting.playernamedied]
    if tp_player then
        for i = 1, 5 do
            wait()
...
Sectionmogui:Button("传送到玩家基地", function()
    local ME = game.Players.LocalPlayer.Character.HumanoidRootPart
    for i, v in pairs(game.Workspace.Properties:GetChildren()) do
        if v.Owner.Value == game.Players[daoting.playernamedied] then
    ...
    local function callback(Text)
        if Text == "确定" then
            ...
            spawn(function()
                ...
                if not game.Players[daoting.playernamedied].Character.Humanoid.SeatPart then
                    repeat
                        task.wait()
                        carTeleport(game.Players[daoting.playernamedied].Character.HumanoidRootPart.CFrame +
                                        Vector3.new(0, -2, 0))
                    until game.Players[daoting.playernamedied].Character.Humanoid.SeatPart
                end
                while task.wait() do
                    for i, v in pairs(game.Workspace.Properties:GetChildren()) do
                        if v.Owner.Value == game.Players.LocalPlayer then
...
Sectionmogui:Button("斧头杀人", function()
    ...
    local KillPlayer = daoting.playernamedied
...
Sectionmogui:Toggle("查看玩家", 'Toggleflag', false, function(state)
    if state then
        game:GetService('Workspace').CurrentCamera.CameraSubject =
            game:GetService('Players'):FindFirstChild(daoting.playernamedied).Character.Humanoid
    else
...
Sectionautobuy:Button("买", function()
    daoting.autobuystop = false
    daoting.autobuyset = lp.Character.HumanoidRootPart.CFrame
    autobuy(l, daoting.autobuyamount)
    task.wait()
    tp(daoting.autobuyset)
end)
Sectionautobuy:Button("停止购买", function()
    daoting.autobuystop = true
    pcall(function()
        daoting.autocsdx:Disconnect();
        daoting.autocsdx = nil;
    end)
...
Sectionautobuy6:Button("买鲨鱼", function()
    ...
    daoting.autobuystop = false
    daoting.autobuyset = CFrame.new(319, 43, 1914)
    autobuy("BagOfSand", 1)
    task.wait(0.1)
    daoting.autobuyset = CFrame.new(317, 43, 1918)
    autobuy('CanOfWorms', 1)
    task.wait(0.1)
    daoting.autobuyset = CFrame.new(322, 43, 1916)
    autobuy('LightBulb', 1)
    tp(daoting.autobuyset)
    ...
    axeConnection = workspace.PlayerModels.ChildAdded:Connect(function(Child)
        ...
        if Main:FindFirstChild 'Mesh' and Main.Mesh.TextureId == 'rbxassetid://273892918' then
            ...
            pcall(function()
                axeConnection:Disconnect();
                axeConnection = nil;
                daoting.boxOpenConnection:Disconnect();
                daoting.boxOpenConnection = nil;
            end);
        end
    end);
    daoting.boxOpenConnection = workspace.PlayerModels.ChildAdded:Connect(function(Child)
        pcall(function()
            wait(.5)
