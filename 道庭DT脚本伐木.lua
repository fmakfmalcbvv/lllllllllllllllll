local DTlib = loadstring(game:HttpGet(("https://pastebin.com/raw/vHxButt8")))()
	...
	DTwin1:Toggle("防误触", "Mistouch", true, function(v)
	    _CONFIGS["防误触开关"] = v;
	end)
	
	DTwin1:Button("关闭脚本", function()
	...
	DTwin1:Button("脚本作者  锋芒阿康", function()
		print("感谢 锋芒阿康")
	end)
	
	DTwin1:Button("脚本合作者  猫猫", function()
		print("感谢 猫猫")
	end)
	
	DTwin1:Button("Woof UI Library  平民", function()
		print("感谢 Step")
	end)
	
	DTwin1:Button("UI提供 紅", function()
		print("感谢 紅")
	end)
	
	DTwin1:Button("部分功能来源 紅", function()
		print("感谢 紅")
	end)
	
	DTwin1:Button("感谢粉丝们的支持", function()
		print("感谢粉丝")
	end)
	
	
	DTwin1:Button("最大支持  你们", function()
		print("感谢 你们")
	end)
	
	
	DTwin2:Slider("步行速度", "步行速度slider", 50, 16, 300, false, function(v)
		_CONFIGS["步行速度"] = v;
	end)
	
	DTwin2:Slider("跳跃力", "跳跃力slider", 50, 50, 300, false, function(v)
		_CONFIGS["跳跃力"] = v;
	end)
	
	DTwin2:Slider("飞行速度","飞行速度slider", 4, 1, 100, false, function(v)
		_CONFIGS["飞行速度"] = tonumber(v)
	end)
	
	DTwin2:Toggle("飞行", "fly", false, function(bool)
	    _CONFIGS["飞行开关"] = bool
	...
	DTwin2:Slider("悬浮高度", "悬浮slider", 0, 0, 300, false, function(v)
		_CONFIGS["悬浮高度"] = v;
	end)
	
	DTwin2:Slider("重力", "重力slider", 198, 0, 300, false, function(v)
		_CONFIGS["重力"] = v;
	end)
	
	DTwin2:Toggle("无限跳", "toggleInfJump", false, function(bool)
	    _CONFIGS["无限跳"] = bool;
	...
	DTwin2:Toggle("穿墙", "toggleNoclip", false, function(bool)
	    _CONFIGS["穿墙开关"] = bool;
	...
	DTwin2:Button("安全自杀--旧(会掉斧头)", function()
	...
	DTwin2:Button("安全自杀--新(不会掉斧头)", function()
	    DT:TP(273, -350, -98)
	end)
	
	DTwin2:Button("点击传送 [工具]", function()
	...
	DTwin2:Toggle("灯光", "light", false, function(bool)
	    if bool then
	...
	DTwin2:Toggle("水上行走", "waterWalk", false, function(bool)
	...
	DTwin2:Slider("相机焦距", "焦距slider", 2000, 100, 2000, false, function(v)
	    _CONFIGS["相机焦距"] = v;
	end)
	
	
	DTwin2:Slider("广角", "广角slider", 70, 70, 120, false, function(v)
	    _CONFIGS["广角"] = v;
	end)
	...
	DTwin3:Slider("物品数量", "buycountslider", 1, 1, 25, false, function(v)
	    getgenv()["物品总数"] = v;
	end)
	
	getgenv()["不想购买"] = false;
	
	DTwin3:Button("停止购买", function()
	    getgenv()["不想购买"] = true;
	    wait(1)
	    getgenv()["不想购买"] = false;
	end)
	...
	DTwin3:Dropdown("反斗城商品", "shop_woodrus", WoodRus_Store, function(WShop)
	...
	DTwin3:Dropdown("蓝图商品", "shop_blueprint", BluePrintStore, function(item)
	...
	DTwin3:Dropdown("盒子车行", "shop_woodrus", Car_Store, function(item)
	...
	DTwin3:Dropdown("家具店商品", "shop_f", Furniture_Store, function(item)
	...
	DTwin3:Dropdown("鲍勃小屋", "shop_shack", Shack_Shop, function(item)
		if item == "炸药 220$" then
			Shack_Shop = "Dynamite"
		end
	...
	DTwin3:Dropdown("艺术品商店", "shop_fineart", FineArts, function(item)
	...
	DTwin3:Dropdown("连接逻辑店", "shop_logic", Logic_Store, function(item)
		if item == "铁丝 205$" then
			Logic_Store = "Wire"
		end
		if item == "按钮 320$" then
	...
	DTwin4:Slider("砍树数量", "砍树slider", 1, 1, 30, false, function(v)
	   getgenv()["带来树数量"] = v;
	end)
	...
	DTwin4:Dropdown("选择树", "select_tree", tree_list, function(list)
		if list == "火山木" then
			select_tree = ("Volcano")
		end
		if list == "蓝木" then
	...
	DTwin4:Toggle("点击出售木材", "clicksell", false, function(v)
	    getgenv()["点击出售木头"] = v;
	...
	DTwin4:Toggle("出售全部木头", "sellallwood", false, function(v)
	    sellAllLog = v;
		local sell = CFrame.new(315.12146, - 0.190167814, 85.0448074);
		local oldpos = DT.LP.Character.HumanoidRootPart.CFrame
	...
	DTwin4:Toggle("点击自动砍树", "clickautocut", false, function(v)
	    _CONFIGS["点击砍树"] = v;
		local Mouse = DT.LP:GetMouse()
	...
	DTwin4:Button("处理木材并加工", function()
	    if getgenv()["点击出售木头"] == true then
	        return library:Notify({
			           Title = "错误!", 
			           Text = "请先关闭点击出售木头!",
	...
	DTwin4:Button("木板1x1 测试", function()
	    if getgenv()["点击出售木头"] == true then
	...
	DTwin4:Toggle("出售全部木板", "sellallplank", false, function(v)
	    sellAllPlank = v;
		local sell = CFrame.new(315.12146, - 0.190167814, 85.0448074);
		local oldpos = DT.LP.Character.HumanoidRootPart.CFrame
	...
	DTwin4:Toggle("拖拽器", "dragmode", false, function(state)
	    	if state then
	...
	DTwin4:Toggle("查看幻影木", "viewendtimetree", false, function(state)
	    	if state then
			local wood
	...
	DTwin4:Button("无眼球砍幻影", function(bool)
		local wood = nil
		local oldpos = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame;
	...
	DTwin5:Dropdown("商店地点", "shop_tp", StoreTP, function(new)
	    StoreTP = new;
	end)
	
	DTwin5:Button("传送!", function()
	...
	DTwin5:Dropdown("木材地点", "wood_tp", WoodTP, function(new)
	    WoodTP = new;
	end)
	...
	DTwin5:Dropdown("其他地点", "other_tp", OtherTP, function(new)
	    OtherTP = new;
	end)
	...
	DTwin5:Dropdown("选择玩家", "player_tp", getgenv()["玩家们"], function(plr)
	    getgenv()["玩家们"] = plr;
	end)
	
	DTwin5:Button("传送到玩家身边!", function() 
	...
	DTwin5:Button("传送到玩家基地!", function() 
	    if type(getgenv()["玩家们"]) == "table" then
	        return DT:NOTIFY("错误", "请先选择玩家", 4)
	    end
	...
	DTwin5:Toggle("查看玩家", "viewPlayer", false, function(state)
	    if state then
	        if type(getgenv()["玩家们"]) == "table" then
	            return DT:NOTIFY("错误", "请先选择玩家", 4)
	        end
	...
	DTwin5:Toggle("查看玩家基地", "viewPlayerBase", false, function(state)
	    if state then
	        for i, v in next, DT.WKSPC.Properties:GetChildren() do
	            if v:FindFirstChild("Owner") and v.Owner.Value == DT.GS("Players")[tostring(getgenv()["玩家们"])] then
	...
	DTwin6:Toggle("白天", "day", true, function(state)
	    Day = state;
	end)
	
	DTwin6:Toggle("黑夜", "night", false, function(state)
	    Night = state;
	end)
	
	DTwin6:Toggle("删除雾", "nofog", true, function(state)
	    NoFog = state;
	end)
	
	DTwin6:Toggle("万圣节", "wsj", false, function(state)
	    DT.LIGHT.Spook.Value = state;
	end)
	
	DTwin6:Toggle("阴影", "shadow", false, function(state)
	    DT.LIGHT.GlobalShadows = state;
	end)
	
	DTwin6:Slider("场景亮度", "bring", 2, 0, 10, false, function(v)
	    DT.LIGHT.Brightness = v
	end)
	
	DTwin6:Separator();
	
	DTwin6:Toggle("删除水", "deletewater", false, function(state)
	    local water = {}
	    if state then
	...
	DTwin6:Toggle("删除岩浆", "deleteLava", false, function(state)
	    if not state then
	...
	DTwin6:Toggle("删除火山巨石", "deleteBoulder", false, function(state)
	    if state then
	...
	DTwin6:Toggle("删除雪山石头", "deleteSnow", false, function(state)
	    if state then
	...
	DTwin6:Toggle("删除鲨鱼斧入口", "deleteDenHatch", false, function(state)
	    if state then
	...
	DTwin6:Button("生成上火山捷径", function()
...
DTwin6:Button("生成去椰子岛的捷径", function()
...
DTwin6:Button("冰木捷径", function()
	...
	DTwin7:Button("免费土地", function() 
	    DT:FreeLand()
	end)
	
	DTwin7:Button("出售土地牌子", function() 
	    local oldtime = tick();
	    local oldpos = DT.LP.Character.HumanoidRootPart.CFrame;
	    local sell = CFrame.new(315.12146, - 0.190167814, 85.0448074);
	...
	DTwin7:Button("最大土地", function() 
	    maxland()
	end)
	
	DTwin7:Separator();
	
	function cooperAxeDupe()                        -- get cooper
	    DT:TP(273, -350, -98)
	    wait(2)
	...
	DTwin7:Button("复制斧头(加载冷却时间过后用，否则有bug)", function() 
	    cooperAxeDupe()
	end)
	...
	DTwin7:Toggle("循环复制斧头", "loopdupeAxe", false, function(state)
	...
	DTwin7:Button("复制基地  需要朋友", function()
	...
	DTwin7:Toggle("木板填充蓝图", "planktoblueprint", false, function(state)
	       if getgenv()["点击出售木头"] == true then
	        return DT:NOTIFY("错误","请先关闭点击出售木头",4)
	    end
		_CONFIGS["填充工具"] = state
	...
	DTwin7:Button("删除位置!", function() 
	    if DT.WKSPC:FindFirstChild("BRING") then
	        DT.WKSPC.BRING:Destroy()
	    end
	end)
	
	DTwin7:Button("获取传送工具!", function() 
	...
	DTwin8:Slider("汽车速度", "carSpeedSlider", 1, 0, 10, false, function(v)
...
spawn(function()
    while task.wait() do
        game:GetService("Workspace").Stores.WoodRUs.Parts.PREMIUMSELECTION.SurfaceGui.TextLabel.Text = "道庭DT付费脚本"
        ...
        game:GetService("Workspace").Stores.WoodRUs.Parts.PREMIUMSELECTION.SurfaceGui.TextLabel.TextColor3 =
            Color3.fromRGB(255, 255, 255)
        wait(1)
        game:GetService("Workspace").Stores.WoodRUs.Parts.PREMIUMSELECTION.SurfaceGui.TextLabel.Text = "伐木大亨最好的脚本作者是 紅"
        game:GetService("Workspace").Stores.WoodRUs.Parts.PREMIUMSELECTION.SurfaceGui.TextLabel.TextColor3 =
            Color3.fromRGB(255, 255, 255)
        wait(2.5)
...
game:GetService("Workspace").Stores.WoodRUs.Parts.OPEN24HOURS.SurfaceGui.TextLabel.Text = "作者: 红毛狗头(副)";
game:GetService("Workspace").Stores.WoodRUs.Parts.OPEN24HOURS.SurfaceGui.TextLabel.TextColor3 =
    Color3.fromRGB(255, 255, 255)
game:GetService("Workspace").Stores.WoodRUs.Parts.SELLWOOD.SurfaceGui.TextLabel.Text = "作者: 锋芒阿康(主)";
game:GetService("Workspace").Stores.WoodRUs.Parts.SELLWOOD.SurfaceGui.TextLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
game:GetService("Workspace").Stores.WoodRUs.Parts.WOODDROPOFF.SurfaceGui.TextLabel.Text = "感谢使用道庭DT";
...
win7:Textbox("购买数量", "请输入!", function(txt)
    DT.autobuyamount = txt
...
win7:Dropdown("选择物品", "这个有用", this_table, function(a)
    if a == '按钮' then
        l = 'Button0'
    elseif a == '控制杆' then
        l = 'Lever0'
	...
	win7:Button("购买", function()
	   DT.autobuystop = false
    DT.autobuyset = lp.Character.HumanoidRootPart.CFrame
    autobuy(l, DT.autobuyamount)
    task.wait()
    tp(DT.autobuyset)
    print("btn3");
end)
...
win1:Toggle("自动扔斧头", "id随便", false, function(state)
    DT.autodropae = true
    if state then
...
win1:Toggle("自动捡斧头", "id", false, function(state)
    print(bool)
    if state then
        DT.autopick = true
...
win1:Button("远程装备斧头", function()
    farAxeEquip()
    print("btn3");
end)
win1:Toggle("斧头炸家(电脑)", "i123", false, function(state)
    print(bool)
    if state then
        DT.whthmose = true
    else
...
win1:Toggle("斧头炸家", "izane便", false, function(state)
    print(bool)
    if state then
        axefily()
    else
...
local win2 = library:CreateTab("存档");
win2:Textbox("选择存档", "请输入!", function(s)
    DT.soltnumber = s
    print(text);
end)
win2:Button("加载存档", function()
...
local win3 = library:CreateTab("处理木头");
win3:Button("半自动处理树1", function()
    wait(0.5)
...
win4:Textbox("你要说的话", "请输入!", function(txt)
    DT.saymege = txt
    print(text);
end)
win4:Textbox("说话次数", "请输入!", function(txt)
    DT.saymount = txt
    print(text);
end)
win4:Button("开始说话", function()
    DT.sayfast = true
...
win4:Button("停止说话", function()
    DT.sayfast = false
    print("btn3");
end)
win4:Toggle("全自动说话", "id19784", false, function(state)
    if state then
        DT.autosay = true
        while task.wait() do
	...
	win4:Dropdown("选择玩家", "用", DT.dropdown, function(v)
	    DT.moneytoplayername = v
	print(item);
end)
win4:Button("刷新列表", function()
    shuaxinlb(true)
    dropdown:SetOptions(DT.dropdown)
    print("btn3");
end)
win4:Textbox("给玩家转钱的数量", "请输入!", function(txt)
    DT.moneyaoumt = txt
    print(text);
end)
win4:Button("开始转钱", function()
    donate(DT.moneytoplayername, DT.moneyaoumt)
    print("btn3");
end)
win4:Button("小工具", function()
...
win4:Toggle("远程打开东西", "77646便", false, function(state)
    if state then
...
win6:Dropdown("选择玩家", "这个有用", DT.dropdown, function(v)
    DT.zlwjia = v
	print(item);
end)
	win6:Button("重置名称", function()
	    shuaxinlb(true)
    Players:SetOptions(DT.dropdown)
    print("btn3");
end)
win6:Textbox("x轴", "请输入!", function(txt)
    DT.zix = txt
    print(text);
end)
win6:Textbox("z轴", "请输入!", function(txt)
    DT.zlz = txt

    print(text);
end)
	win6:Button("整理工具", function()
...
win6:Dropdown("选择玩家", "这个1", DT.dropdown, function(v)
    DT.cswjia = v
	print(item);
	
end)
win6:Button("重置玩家名称", function()
     shuaxinlb(true)
    Players:SetOptions(DT.dropdown)
       print("btn3");
    
end)
win6:Button("设置传送点", function()
    pcall(function()
    ...
    print("btn3");
end)win6:Button("删除传送点", function()
    pcall(function()
        game.Workspace.DTBasedropCord:Destroy();
        DT.itemset = nil
    end)
    ...
    print("btn3");
end)win6:Button("获得传送工具", function()
...
win6:Toggle("选择传送物品", "64518976734随便", false, function(state)
    print(bool)
    if state then
        ClickToSelectClick = Mouse.Button1Up:Connect(function()
            Clicked = Mouse.Target
...
win6:Toggle("框选物品", "64664346199随便", false, function(state)
    if state then
...
win6:Toggle("带木板", "id随便", false, function(state)
    DT.xzemuban = state

    print(bool)

	end)
	win6:Button("开始传送物品", function()
	    if DT.itemset == nil then
        return notify("道庭DT", "请你放传送点", 4)
    end
    DT.cskais = true
...
win6:Button("停止", function()
    DT.cskais = false
    print("btn3");
end)

local win8 = library:CreateTab("复仇玩家");
win8:Dropdown("选择玩家", "这个有用", DT.dropdown, function(v)
    DT.playernamedied = v
	print(item);
end)
win8:Button("刷新玩家", function()
    shuaxinlb(true)
    dropdown:SetOptions(DT.dropdown)
    print("btn3");
end)
win8:Button("传送玩家身边", function()
    ...
    print("btn3");
end)win8:Button("传送到玩家基地", function()
     local ME = game.Players.LocalPlayer.Character.HumanoidRootPart
    for i, v in pairs(game.Workspace.Properties:GetChildren()) do
       ...
       print("btn3");
end)win8:Button("汽车传送到到玩家基地", function()
    for i, v in pairs(game.Workspace.Properties:GetChildren()) do
        if v.Owner.Value == game.Players[DT.playernamedied] then
    ...
    print("btn3");
end)win8:Button("汽车踢玩家", function()
    local ME = game.Players.LocalPlayer.Character.HumanoidRootPart
    ...
    print("btn3");
end)win8:Button("斧头杀玩家", function()
    local tool = getTool()
    if not tool then
        return notify("道庭DT", "你需要斧头", 4)
    end
